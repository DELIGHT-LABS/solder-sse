...
import { createStream } from './index.js';
...
describe('createStream', () => {
	it('subscribes for as long as the owner lives and the URL is set, and follows a URL change', () => {
		...
		const [url, setUrl] = createSignal<string | undefined>('/a');
		...
		expect(onSwitch).toHaveBeenCalledWith(undefined, '/a');
		expect(stream.status()).toBe('connecting');
		expect(stream.link()).toBeUndefined();
