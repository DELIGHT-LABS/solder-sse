...
describe('subscribe', () => {
	...
	it('a late joiner whose replayed handler throws is reported and can still leave', () => {
		...
		const stop = sub('/throws', { onStatus });
		expect(onStatus).toHaveBeenCalledWith('live');
		expect(error).toHaveBeenCalledWith('[solder-sse] subscriber threw', expect.any(Error));
		expect(solder.inspect('/throws')?.subscribers).toBe(2);
		stop();
		expect(solder.inspect('/throws')?.subscribers).toBe(1);
		error.mockRestore();
	});
	...
	it('unsubscribe lingers, then closes the shared source', () => {
		...
		expect(solder.inspect('/t5')).toBeUndefined();
	});
...
describe('resume', () => {
	...
	it('resync is delivered and clears the rejected cursor', () => {
		...
		expect(solder.inspect('/r4')?.lastEventId).toBeUndefined();
		FakeEventSource.last().die();
		vi.advanceTimersByTime(1_000);
...
describe('reconnect edge', () => {
	...
	it('keeps the rotation age a ping announces as information only', () => {
		...
		expect(solder.inspect('/rc2')?.maxAgeMs).toBeUndefined();
	});
});
